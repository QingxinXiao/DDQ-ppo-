...
from deep_dialog.qlearning import RF
...
class AgentRF(Agent):
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, params=None):
        ...
        self.rf = RF(self.state_dimension, self.hidden_size, self.num_actions).to(DEVICE)
        self.target_rf = RF(self.state_dimension, self.hidden_size, self.num_actions).to(DEVICE)
        self.target_rf.load_state_dict(self.rf.state_dict())
        self.target_rf.eval()

        # self.optimizer = optim.RMSprop(self.rf.parameters(), lr=1e-3)
        self.optimizer = torch.optim.Adam(self.rf.parameters(), lr=1e-3)
        # self.G = 0
    ...
    def run_policy(self, representation):
        ...
        if random.random() < self.epsilon:
            ...
            if self.warm_start == 1:
                ...
                return self.RF_policy(representation)
    ...
    def RF_policy(self, state_representation):
        """ Return action from DQN"""

        with torch.no_grad():
            action = self.rf.predict(torch.FloatTensor(state_representation))
        return action
    ...
    def train(self, batch_size=1, num_batches=100):
        ...
        for iter_batch in range(num_batches):
            ...
            for iter in range(len(self.running_expereince_pool) / (batch_size)):
                ...
                log_prob = torch.log(self.target_rf(torch.FloatTensor(state)).gather(1, action))
                G = self.gamma * G + reward
                # loss = F.mse_loss(-log_prob, G)
                loss = -log_prob * G
                # print "loss:{}".format(loss)
                # loss = (-log_prob * G).detach().numpy()
                loss.sum().backward()
            self.optimizer.step()

                # state_value = self.rf(torch.FloatTensor(batch.state)).gather(1, torch.tensor(batch.action))
    ...
    def save(self, filename):
        torch.save(self.rf.state_dict(), filename)

    def load(self, filename):
        self.rf.load_state_dict(torch.load(filename))

    def reset_rf_target(self):
        self.target_rf.load_state_dict(self.rf.state_dict())