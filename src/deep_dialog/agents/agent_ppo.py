...
from deep_dialog.qlearning import PPO
...
class AgentPPO(Agent):
    def __init__(self, movie_dict=None, act_set=None, slot_set=None, params=None):
        ...
        self.actor_ppo = PPO(self.state_dimension, self.hidden_size, self.num_actions).to(DEVICE)
        self.critic_ppo = PPO(self.state_dimension, self.hidden_size, self.num_actions).to(DEVICE)
    ...
    def PPO_policy(self, state_representation):
        ...
        with torch.no_grad():
            action = self.actor_ppo.predict(torch.FloatTensor(state_representation))
        return action
    ...
    def compute_advantage(self, g, l, d):
        d = d.detach().numpy()
        a_list = []
        a = 0.0
        for i in d[::-1]:
            ...
            a_list.append(a)
            a_list.reverse()
            return torch.tensor(a_list, dtype = torch.float)

    def ppo_train(self, batch_size=1, num_batches=100):
        ...
        for iter_batch in range(num_batches):
            ...
            for iter in range(len(self.running_expereince_pool) / batch_size):
                ...
                # td_delta = td_target
                old_log_probs = torch.log(self.actor_ppo(torch.FloatTensor(batch.state)).gather(1, torch.tensor(batch.action))).detach()
                advantage = self.compute_advantage(self.gamma, self.lmbda, td_delta)

                log_probs = torch.log(self.actor_ppo(torch.FloatTensor(batch.state)).gather(1, torch.tensor(batch.action)))
                ratio = torch.exp(log_probs - old_log_probs)
                surr1 = ratio * advantage
                surr2 = torch.clamp(ratio, 1 - self.eps, 1 + self.eps) * advantage
                actor_loss = torch.mean(-torch.min(surr1, surr2))
                critic_loss = torch.mean(F.mse_loss(self.critic_ppo(torch.FloatTensor(batch.state)), td_target.detach()))
                ...
                # state_value = self.actor_ppo(torch.FloatTensor(batch.state)).gather(1, torch.tensor(batch.action))
                # # max Q
                # next_state_value, _ = self.target_ppo(torch.FloatTensor(batch.next_state))
    ...
    def save(self, filename):
        torch.save(self.actor_ppo.state_dict(), filename)

    def load(self, filename):
        self.actor_ppo.load_state_dict(torch.load(filename))

    def reset_ppo_target(self):
        self.actor_ppo.load_state_dict(self.actor_ppo.state_dict())
        self.critic_ppo.load_state_dict(self.critic_ppo.state_dict())
